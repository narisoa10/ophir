...
type ReverseRpcResult = {
  status: string;
  reconciliation_id?: string;
  transfer_operation_id?: string | null;
};

type ReverseDatabase = {
  reverseInternalTransferResolution(
    userId: string,
    reconciliationId: string,
  ): Promise<{ data: ReverseRpcResult | null; errorMessage: string | null }>;
};

type HandlerDependencies = {
  authenticateRequest: (request: Request) => Promise<AuthenticatedUser | null>;
  createDatabase: () => ReverseDatabase | null;
};
...
function mapRpcException(message: string): Response {
  ...
  if (lower.includes("transfer_operation_not_found")) {
    return errorResponse(404, "not_found");
  }
...
function defaultCreateDatabase(): ReverseDatabase | null {
  ...
  return {
    async reverseInternalTransferResolution(userId, reconciliationId) {
...
export function createPlaidReverseInternalTransferHandler(
  ...
  return async (request: Request): Promise<Response> => {
    ...
    if (data.status === "reversed" || data.status === "already_reversed") {
      if (
