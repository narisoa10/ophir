...
comment on function public.plaid_reconcile_transaction_operation_projections(
    ...
    'Fenced service-role reconciliation from authoritative raw Plaid transactions to projection lifecycle rows. Does not read or write public.operations.';

create or replace function public.plaid_continue_transaction_projection_job(
    p_plaid_item_id uuid,
    p_lease_token uuid
)
returns jsonb
...
begin
    ...
    returning true into v_continued;

    if coalesce(v_continued, false) then
        return jsonb_build_object('status', 'continued');
    end if;
...
comment on function public.plaid_continue_transaction_projection_job(uuid, uuid) is
    'Returns a live projection job lease to pending state when bounded reconciliation processed a chunk and more dirty rows remain. Service-role only.';

create or replace function public.plaid_drop_transaction_projection_job(
    p_plaid_item_id uuid,
    p_lease_token uuid
)
returns jsonb
...
begin
    ...
    where plaid_item_id = p_plaid_item_id
      ...
      and lease_token = p_lease_token
      and lease_expires_at > now()
    returning true into v_dropped;

    if coalesce(v_dropped, false) then
...
comment on function public.plaid_drop_transaction_projection_job(uuid, uuid) is
    'Drops a live projection job lease when the Item/job is no longer actionable. Service-role only.';
...
revoke all on function public.plaid_continue_transaction_projection_job(uuid, uuid)
from public;
revoke all on function public.plaid_continue_transaction_projection_job(uuid, uuid)
from anon;
revoke all on function public.plaid_continue_transaction_projection_job(uuid, uuid)
from authenticated;
grant execute on function public.plaid_continue_transaction_projection_job(uuid, uuid)
to service_role;

revoke all on function public.plaid_drop_transaction_projection_job(uuid, uuid)
from public;
revoke all on function public.plaid_drop_transaction_projection_job(uuid, uuid)
from anon;
revoke all on function public.plaid_drop_transaction_projection_job(uuid, uuid)
from authenticated;
grant execute on function public.plaid_drop_transaction_projection_job(uuid, uuid)
to service_role;
