...
final class RecurringCommitmentProblemService {
  ...
  List<FinancialProblem> detect(List<FinancialDeviation> deviations) {
    final required = _deviationsFor(
      deviations,
      FinancialProblemCatalog.fixedCommitmentPressure.requiredDeviationTypes,
    );
    if (required.isEmpty) {
      return const [];
    }
    ...
    return [
      resultBuilder.detected(
        rule: FinancialProblemCatalog.fixedCommitmentPressure,
        requiredDeviations: required,
        supportingDeviations: _deviationsFor(
          deviations,
          FinancialProblemCatalog
