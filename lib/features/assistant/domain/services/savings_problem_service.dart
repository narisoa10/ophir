...
final class SavingsProblemService {
  const SavingsProblemService({
  ...
  List<FinancialProblem> detect(List<FinancialDeviation> deviations) {
    final required = _deviationsFor(
      deviations,
      FinancialProblemCatalog.weakSavingsCapacity.requiredDeviationTypes,
    );
    if (required.isEmpty) {
      return const [];
    }
    ...
    return [
      resultBuilder.detected(
        rule: FinancialProblemCatalog.weakSavingsCapacity,
        requiredDeviations: required,
        supportingDeviations: _deviationsFor(
          deviations,
          FinancialProblemCatalog.weakSavingsCapacity.supportingDeviationTypes,
        ),
