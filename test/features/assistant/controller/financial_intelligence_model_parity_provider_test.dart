...
void main() {
  group('financialIntelligenceModelParityProvider', () {
    test(
      'builds parity snapshot from legacy and intelligence models',
      () async {
        ...
        final snapshot = switch (result) {
          ...
          Failure() => fail('Expected intelligence model parity snapshot'),
        };
        ...
        expect(
          snapshot
              .resultFor(FinancialIntelligenceModelParityMetric.assetBuilding)
              .intelligenceValue,
          300,
        );
        expect(
        ...
        expect(
          snapshot
              .resultFor(FinancialIntelligenceModelParityMetric.debtReduction)
              .intelligenceValue,
          600,
        );
      },
    ...
    test('provider boundary does not feed runtime or diagnostics', () {
      final paritySource = File(
        'lib/features/assistant/controller/'
        'financial_intelligence_model_parity_provider.dart',
      ).readAsStringSync();
      ...
      expect(
        paritySource,
        contains('financialIntelligenceDiagnosticsProvider'),
      );
      expect(
        paritySource,
    ...
    test('does not change existing Assistant recommendation output', () async {
      ...
      await container.read(financialIntelligenceModelParityProvider.future);
  ...
  return ProviderContainer(
    overrides: [
      ...
      operationRepositoryProvider.overrideWithValue(
        _FakeOperationRepository(operations: _operations(account.id, now)),
      ),
      categoryRepositoryProvider.overrideWithValue(
        _FakeCategoryRepository(categories: _categories()),
      ),
    ],
  );
}
...
List<Operation> _operations(String accountId, DateTime now) {
  return [
    _operation(
      id: 'income',
